...
class BaseDataset(Dataset):
    ...
    def forward_img(self, index):
        ...
        img_flip, mask_flip, kp_flip, sfm_pose_flip = self.mirror_image(img, mask, kp, sfm_pose)
        img_flip = np.transpose(img_flip, (2, 0, 1))

        # Normalize kp to be [-1, 1]
        img_h, img_w = img.shape[:2]
        kp_norm, sfm_pose = self.normalize_kp(kp, sfm_pose, img_h, img_w)
        kp_norm_flip, sfm_pose_flip = self.normalize_kp(kp_flip, sfm_pose_flip, img_h, img_w)

        # Finally transpose the image to 3xHxW
        img = np.transpose(img, (2, 0, 1))
        return img, kp_norm, mask, sfm_pose, img_flip, kp_norm_flip, mask_flip, sfm_pose_flip, cad_index
    ...
    def __getitem__(self, index):
        img, kp, mask, sfm_pose, img_flip, kp_flip, mask_flip, sfm_pose_flip, cad_index = self.forward_img(index)
        ...
        elem['img_flip'] = img_flip
        elem['mask_flip'] = mask_flip
        elem['kp_flip'] = kp_flip
        elem['sfm_pose_flip'] = np.concatenate(sfm_pose_flip)
