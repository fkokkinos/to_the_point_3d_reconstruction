...
flags.DEFINE_string('results_vis_dir', osp.join(cache_path, 'results_vis'),
                    'Directory where intermittent results will be saved')
flags.DEFINE_string('results_eval_dir', osp.join(cache_path, 'evaluation'),
                    'Directory where evaluation results will be saved')
...
class Tester():
    ...
    def save_current_visuals(self):
        ...
        for k in visuals:
            ...
            to_save = visuals[k].permute(1, 2, 0).cpu().numpy()
            to_save = 255 * to_save  # Now scale by 255
            to_save = to_save.astype(np.uint8)
            io.imsave(img_path, to_save)
        self.vis_iter += 1
