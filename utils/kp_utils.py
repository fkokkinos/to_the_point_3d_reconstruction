...
def vis_part_heatmaps(response_maps, threshold=0.5, prefix=''):
    ...
    for b in range(B):
        for k in range(K):
            ...
            response_map = cv2.applyColorMap((response_map * 255.0).astype(np.uint8), cv2.COLORMAP_HOT)[:, :,
                           ::-1]  # BGR->RGB
    ...
    results = None
    for k in range(K):
        map_viz_single = vutils.make_grid(part_response[:, k, :, :, :].squeeze() / 255.0, normalize=False,
                                          scale_each=False)
        if (results is None):
