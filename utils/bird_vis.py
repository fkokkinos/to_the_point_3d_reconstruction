...
class VisRenderer(object):
    ...
    def __init__(self, img_size, num_verts, background_color=0, lights=None, t_size=3):
        self.renderer = NeuralRenderer(img_size, background_color=background_color, lights=lights)
    ...
    def __call__(self, verts, faces, cams=None, K=None, texture=None, rend_mask=False, atlas=True):
        """
        ...
        """
        if texture is None:
            texture = self.default_tex.clone().detach().repeat(verts.shape[0], 1, 1)
        ...
        if rend_mask:
            rend = self.renderer.forward(verts, faces, cams, K)
            rend = rend.repeat(3, 1, 1)
            rend = rend.unsqueeze(0)
        else:
            rend = self.renderer.forward(verts, faces, cams, K, texture)[0]
        rend.clamp_(0, 1)
        return rend
    ...
    def diff_vp(self,
        ...
        new_cam = quaternion_to_angle_axis(new_quat)
        rend_img = self.__call__(verts[None], faces[None], cams=new_cam[None], K=K[None], texture=texture, atlas=atlas)
        if kp_verts is None:
            return rend_img[0]
        else:
...
def kp2im(kp, img, radius=None, manual_colors=None):
    """
    ...
    kp_img = draw_kp(kp, img, radius=radius, manual_colors=manual_colors)

    return kp_img


def draw_kp(kp, img, radius=None, manual_colors=None):
    """
    ...
    """
    if radius is None:
        radius = max(3, (np.mean(img.shape[:2]) * 0.005).astype(int))
